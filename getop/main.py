"""getop — read-only troubleshooting / debug / stats CLI for Google Gemini Enterprise.
...
# ---- command groups (implemented in getop/commands/) -----------------------
...
def _build_commit() -> str:
    ...
    try:
        from getop._build import COMMIT  # generated by hatch_build.py
...
def version(
    ...
    try:
        release = pkg_version("getop")
    except PackageNotFoundError:
        from getop import __version__ as release

    data = {"version": release, "tag": f"v{release}", "commit": _build_commit()}
    if as_json:
        from getop.render import emit_json

        emit_json(data)
    else:
        print(f"getop {data['version']} ({data['tag']}, commit {data['commit']})")
...
def _installed_version() -> str:
    ...
    try:
        return pkg_version("getop")
    except PackageNotFoundError:
        from getop import __version__

        return __version__


def _latest_version(timeout: float = 5.0) -> str | None:
    """Latest getop version on PyPI, or None if the check fails (offline etc.)."""
    import json
    import urllib.request

    try:
        with urllib.request.urlopen(
            "https://pypi.org/pypi/getop/json", timeout=timeout
        ) as resp:
...
def _install_method() -> tuple[str, list[str] | None]:
    """Detect how getop was installed and the command to upgrade it.
    ...
    if "/pipx/venvs/" in prefix:
        return "pipx", ["pipx", "upgrade", "getop", "--pip-args=--no-cache-dir"]
    if "/uv/tools/" in prefix or "/uv/tools" in prefix:
        return "uv tool", ["uv", "tool", "upgrade", "--no-cache", "getop"]
    return "pip", [
        sys.executable, "-m", "pip", "install", "--upgrade", "--no-cache-dir", "getop"
    ]
...
def update(
    ...
    """Update getop to the latest release."""
    import subprocess

    from getop.render import console, emit_json, err_console
    ...
    if not outdated:
        console.print(f"getop is up to date ([bold]{current}[/bold]).")
        return
    ...
    if check:
        ...
        return
    if argv is None:
        console.print("Upgrade getop with your package manager.")
        return
    ...
    console.print(
        ...
        "propagating the release. Run [bold]getop update[/bold] again shortly."
    )
    if output:
...
def run() -> None:
    ...
    # google.auth warns that user ADC has no quota project; getop sets one
    ...
    # (e.g. preview ContentConfig values); getop renders them numerically.
    warnings.filterwarnings(
        "ignore",
    ...
    try:
        ...
        if "serviceusage" in message or "USER_PROJECT_DENIED" in str(exc.errors):
            ...
            err_console.print(
                "[yellow]Hint:[/yellow] getop needs roles/discoveryengine.viewer, "
